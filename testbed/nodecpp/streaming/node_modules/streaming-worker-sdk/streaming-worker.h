...
public:
  string name;
  string data;
  MessageSend(string name, string data) : name(name), data(data){}
};
...
class StreamingWorker : public AsyncProgressWorker {
 public:
  ...
  PCQueue<MessageSend> fromNode;

 protected:

  void writeToNode(const AsyncProgressWorker::ExecutionProgress& progress, MessageSend & msg){
    toNode.write(msg);
...
private:
  void drainQueue() {
    ...
    std::deque<MessageSend> contents;
    toNode.readAll(contents);

    for(MessageSend & msg : contents) {
        v8::Local<v8::Value> argv[] = {
...
class StreamWorkerWrapper : public Nan::ObjectWrap {
 ...
 private:
  ...
  static NAN_METHOD(sendToAddon) {
    ...
    obj->_worker->fromNode.write(MessageSend(*name, *data));
  }
